...
print ('\n\nMuDog (MuBot ver 4.0) - MURAVES Experiment')
...
dir = '/home/DatiTB/' + sys.argv[1] + '/'
runFile = '/home/DatiTB/DTC/RI_' + sys.argv[1] + '.txt'
...
arg = ['./sendDTC', muClient, str(pedSr), str(adcSr)]	
subprocess.call(arg)
...
while True :
	...
	DAC10 = []
	DAC10file = open('/home/DatiTB/DTC/WP_' + sys.argv[1], 'r')
	s = DAC10file.readlines()
	DAC10file.close()
	for ii in range(16) :
		DAC10.append(s[ii].split('\t'))
	dac10List = DAC10[0][1].replace('\n', '\t') +  DAC10[1][1].replace('\n', '\t') + DAC10[2][1].replace('\n', '\t') + DAC10[3][1].replace('\n', '\t') + DAC10[4][1].replace('\n', '\t') + DAC10[5][1].replace('\n', '\t') + DAC10[6][1].replace('\n', '\t') + DAC10[7][1].replace('\n', '\t') + DAC10[8][1].replace('\n', '\t') + DAC10[9][1].replace('\n', '\t') + DAC10[10][1].replace('\n', '\t') + DAC10[11][1].replace('\n', '\t') + DAC10[12][1].replace('\n', '\t') + DAC10[13][1].replace('\n', '\t') + DAC10[14][1].replace('\n', '\t') + DAC10[15][1].replace('\n', '\t')
	SCstring = SCstring + dac10List

	inputF = open('/home/DatiTB/DTC/HVtemp_' + sys.argv[1], 'r')
	s = inputF.readline()
	SCstring = SCstring + s
	inputF.close()

	inputF = open('/home/DatiTB/DTC/MUXtemp_' + sys.argv[1], 'r')
	s = inputF.readline()
	SCstring = SCstring + s
	inputF.close()
	...
	arg = ['gzip', '-f', '/home/DatiTB/' + sys.argv[1] + '/SLOWCONTROL_run' + str(ID-1), '/home/DatiTB/' + sys.argv[1] + '/LOG_run' + str(ID-1), '/home/DatiTB/' + sys.argv[1] + '/CONTEGGI_run' + str(ID-1)]
	subprocess.call(arg)

	for i in range (1, pedSr+1):
		arg = ['gzip', '-f', '/home/DatiTB/' + sys.argv[1] + '/pedData_evts' + str(pedSr*10000) + '_run' + str(ID-1) + '_sr' + str(i)]
		subprocess.call(arg)
