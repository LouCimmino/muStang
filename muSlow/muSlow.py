...
light = 'RED'
while light == 'RED' :
	arg = ['cp', '/home/DatiTB/DTC/ENV.txt', 'ENV.txt']
	subprocess.call(arg)
	try :
		#outFile = open('/home/DatiTB/DTC/ENV.txt','r')
		outFile = open('ENV.txt','r')
		s = outFile.readlines()
		outFile.close()
		light = 'GREEN'
	except :
...
try:
	while True:
		...
		if (TMP>=19 and TMP<=32 and (25-DEW)>2) : WP_set = 25
		if (TMP>=13 and TMP<=22 and (20-DEW)>2) : WP_set = 20
		if (TMP>=8 and TMP<=18 and (15-DEW)>2) : WP_set = 15
		if (TMP>=5 and TMP<=13 and (10-DEW)>2) : WP_set = 10
		if (TMP>=1 and TMP<=9 and (6-DEW)>2) : WP_set = 6

		if (TMP>19 and TMP<32 and (25-DEW)>2 and WP == 25) :
			WP_set = 25
		if (TMP>13 and TMP<22 and (20-DEW)>2 and WP == 20) :
			WP_set = 20
		if (TMP>8 and TMP<18 and (15-DEW)>2 and WP == 15) :
			WP_set = 15
