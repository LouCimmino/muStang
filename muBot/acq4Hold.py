...
for dac10 in range (0,1,1):
	...
	while (dac8 >= 255):
		...
		inputF = open('Conf/EASI_Slow_Control_' + sys.argv[1] + '.txt', 'r')
		s = inputF.readlines()
		inputF.close()
		s[4] = '0005\n'
		#s[4] = '0009\n'

		outputF = open('Conf/EASI_Slow_Control_' + sys.argv[1] + '.txt', 'w')
		for line in s:
			outputF.write(line)
		outputF.close()
...
#		input("Press to Stop...")
		while(runCounter<1):
			...
			outputF.write(str(int(round(time.time()*1000))))
			outputF.close()
			arg = ['mv', '/home/muBot/slaveData', '/home/muray/CosmicRun/holdData_evts' + str(evts) + '_sk_' + sys.argv[1]  + '_run_' + sys.argv[2]]
