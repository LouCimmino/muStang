...
void* fileDownload(void *ptr, int argc, char *argv[]);
...
//----------------------------
				...
				for (ii=2; ii<argc-1; ii++) {
					...
					if (pktCount >= 39) {
						...
						printf("  STOP: ");
						mtime_stamp();
						if (badpack > 0) fileDownload( &(arg){buff[tr], buff_tss, buff_trg, size_buff-badpack-1, 1}, argc, argv);
						pthread_exit(NULL);
						return 0 ;
					} 
				}
	...
	printf("  STOP: ");
	mtime_stamp();
	fileDownload( &(arg){buff[tr], buff_tss, buff_trg, size_buff-1, 1}, argc, argv);
	pthread_exit(NULL);
	return 0 ;
...
void* fileDownload(void *ptr, int argc, char *argv[])
{
	...
	sprintf(name, "/home/DatiTB/%s/slaveData", argv[18]);
	f = fopen(name, "a+");
