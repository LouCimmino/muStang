...
//----------------------------
				...
				Npato = 0;
				for(ii=0; ii<12; ii++) trgArr[ii] = 0;
				for(ii=0; ii<12; ii++) addrArr[ii] = 0;
				for(ii=0; ii<12; ii++) flineArr[ii] = 0;
				...
				for (ii=2; ii<argc; ii++) {
					...
					if (pktCount > 39) {
						trgArr[Npato] = kk; 
						addrArr[Npato] = addr;
						flineArr[Npato] = pktCount;
						Npato = Npato + 1;
					}
				}
				printf("---> PATO  LETTE : %d\n", Npato);
				if ((Npato > 0) && (Npato < 12)) {
					for(ii=0; ii<Npato; ii++) { 
						printf("%d\t%d\t%d\n", trgArr[ii], addrArr[ii], flineArr[ii]);
					}
					printf("\n");
				}
				++gotOne22;
		...
		//buff_tss[size_buff_tss] = mtime_stamp_d();
		++size_buff_tss;
		//printf("kk : %d\ttss : %d", kk, size_buff_tss);
		if (kk >= num_evt)
			break;
	...
	//}
	fileDownload( &(arg){buff[tr], buff_tss, size_buff-1, 1});
	pthread_exit(NULL);
	return 0 ;
...
void* fileDownload(void *ptr)
{
	...
	sprintf(name, "/home/muBot/slaveData");
	
	f = fopen(name, "a+");

	while (i <= x->size_buffer) {
		while (ck == 0) {
			if ((x->buffer[i] == 0x25555) && (x->buffer[i+1] == 0x0)) 
				i = i+2;
			else ck = 1;
		}
		ck = 0;
